...
while:
    ...
    #Se a tecla for A, delay = 50ms
    li      $t2, 'A'
    beq     $t1, $t2, setDouble

    #Se a tecla for N, delay = 100ms
    li      $t2, 'N'
    beq     $t1, $t2, setNormal
...
setDouble:
    li      $s4, 50                             # 50 ms => tick a 20Hz
    j resume

setNormal:
    li      $s4, 100                            # 100ms => tick a 20Hz

resume:

    # Delay à velocidade definida
    move    $a0, $s4
    jal     delay                               # delay()
