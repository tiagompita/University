...
int PersonGetId(const Person *p) {
  return p->id;
}

// Clone a person.
Person *PersonClone(const Person *p) {
  return PersonCreate(p->firstName, p->lastName, p->birthDate.year, p->birthDate.month, p->birthDate.day);
}