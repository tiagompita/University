...
public class TicketBookingApp {
	...
	private static void exercise2(PrintStream out) {
		...
		try {
            Scanner scanner = new Scanner(new File("C:\\Users\\tiago\\Desktop\\ExamePratico\\EP\\src\\ticket_bookings.txt"));
            ...
            while (scanner.hasNext()) {
                ...
                if (line.startsWith("Booking")) {
                    ...
                    int numTickets = Integer.parseInt(parts[2].trim());
                    int reference = tbs.bookTickets(eventId, numTickets);

                    if (reference < 0) {
                        out.println("** No more tickets available for event " + eventId);
                    }
                }
            }
            scanner.close();
        ...
        tbs.getEventOccurrences().stream()
            .filter(e -> e.getTicketsAvailable() > 0)
            .sorted(Comparator.comparing(EventOccurrence::getDateTime))
            .limit(10)
            .forEach(out::println);

        out.println("\n** Number of tickets sold by event type:");
        
        Map<String, Integer> ticketsSoldByType = new HashMap<>();
        for (EventOccurrence occurrence : tbs.getEventOccurrences()) {
            String eventType = occurrence.getEvent().getClass().getSimpleName();
            int ticketsSold = occurrence.getPlace().getCapacity() - occurrence.getTicketsAvailable();
            ticketsSoldByType.put(eventType, ticketsSoldByType.getOrDefault(eventType, 0) + ticketsSold);
        }
